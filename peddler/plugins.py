...
class BasePlugin:
    """
    Peddler plugins are defined by a name and an object that implements one or more of the
    ...
    Peddler templates. For instance, to add "somecontent" to a template that includes '{{
    ...
    `command` (click.Command): if a plugin exposes a `command` attribute, users will be able to run it from the command line as `peddler pluginname`.
    """
...
class EntrypointPlugin(BasePlugin):
    """
    Entrypoint plugins are regular python packages that have a 'peddler.plugin.v0' entrypoint.

    The API for Peddler plugins is currently in development. The entrypoint will switch to
    'peddler.plugin.v1' once it is stabilised.
    """

    ENTRYPOINT = "peddler.plugin.v0"
...
class OfficialPlugin(BasePlugin):
    ...
    def __init__(self, name: str):
        self.module = importlib.import_module("peddler{}.plugin".format(name))
...
class DictPlugin(BasePlugin):
    ROOT_ENV_VAR_NAME = "PEDDLER_PLUGINS_ROOT"
    ROOT = os.path.expanduser(
        os.environ.get(ROOT_ENV_VAR_NAME, "")
    ) or appdirs.user_data_dir(appname="peddler-plugins")
...
def enable(config: Dict[str, Any], name: str) -> None:
    if not is_installed(name):
        raise exceptions.PeddlerError("plugin '{}' is not installed.".format(name))
