...
@click.group(
    name="config",
    short_help="Configure OpenCart",
    help="""Configure OpenCart and store configuration values in $PEDDLER_ROOT/config.yml""",
)
...
def save(
    ...
    if set_vars:
        peddler_config.merge(config, dict(set_vars), force=True)
    for key in unset_vars:
        config.pop(key, None)
    peddler_config.save_config_file(context.root, config)
...
def render(context: Context, extra_configs: List[str], src: str, dst: str) -> None:
    ...
    for extra_config in extra_configs:
        peddler_config.merge(
            config, peddler_config.load_config_file(extra_config), force=True
        )
